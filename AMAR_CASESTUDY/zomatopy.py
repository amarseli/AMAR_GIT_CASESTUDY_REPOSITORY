...
class Zomato:
    ...
    def validatelocation(self, location):
        ...
        try:
            print('Maintained location is :: ', location)
            cities_data = pd.read_json('cities.json')
            if cities_data is not None:
                tier1_cities = cities_data["data"]["tier1"]
                tier2_cities = cities_data["data"]["tier2"]
        ...
        locjsonresp     = '{ "data": { "validate_location": "' + location_validity + '" } }'
    ...
    def get_cuisineID(self, cuisine, cusineDict):
        cusineId = 0
        for key, value in cusineDict.items():
            if(value == cuisine):
                cusineId = key
                break
        print('cusineID = ', cusineId)
        return cusineId
    ...
    def get_location(self, query="", limit=5):
        ...
        Returns a list of Restaurant IDs.
        """
        print('STR_LIMIT IS ALPHA :: ', limit)
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
